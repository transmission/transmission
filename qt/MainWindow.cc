...
void MainWindow::refreshActionSensitivity()
{
    ...
    auto const any_paused = std::any_of(std::begin(torrents), std::end(torrents), [](auto const* tor)
        {
            return tor->isPaused();
        });
    auto const any_not_paused = std::any_of(std::begin(torrents), std::end(torrents), [](auto const* tor)
        {
            return !tor->isPaused();
        });
