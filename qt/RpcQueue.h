...
class RpcQueue : public QObject
{
   ...
   public:
    ...
    void setTolerateErrors(bool tolerate_errors = true) { tolerate_errors_ = tolerate_errors; }
   ...
   private:
    ...
    template <typename Func, typename std::enable_if<std::is_same_v<
                                 typename std::invoke_result_t<Func, RpcResponse const&>,
                                 RpcResponseFuture>>::type* = nullptr>
    QueuedFunction normalizeFunc(Func const& func)
    {
        return [func](RpcResponseFuture const& r) { return func(r.result()); };
    }

    // closure without argument (first step), takes nothing and returns new future
    template <typename Func,
              typename std::enable_if<std::is_same_v<typename std::invoke_result_t<Func>,
                                                     RpcResponseFuture>>::type* = nullptr>
    QueuedFunction normalizeFunc(Func const& func)
    {
        return [func](RpcResponseFuture const&) { return func(); };
    }

    // closure without return value ("auxiliary"), takes response and returns nothing -- internally
    // we reuse the last future
    template <typename Func,
              typename std::enable_if<std::is_same_v<
                  typename std::invoke_result_t<Func, RpcResponse const&>, void>>::type* = nullptr>
    QueuedFunction normalizeFunc(Func const& func)
    {
    ...
    // closure without argument and return value, takes nothing and returns nothing -- next function
    // will also get nothing
    template <typename Func, typename std::enable_if<std::is_same_v<
                                 typename std::invoke_result_t<Func>, void>>::type* = nullptr>
    QueuedFunction normalizeFunc(Func const& func)
    {
        return [func](RpcResponseFuture const& r) {
            func();
    ...
    {
        return [func](RpcResponseFuture const& r) { func(r.result()); };
    }
    ...
    ErrorHandlerFunction normalizeErrorHandler(Func const& func)
    {
        return [func](RpcResponseFuture const&) { func(); };
    }
