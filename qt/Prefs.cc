        ...
        switch (Items[i].type)
        {
            ...
            case CustomVariantType::SortModeType:
            {
                ...
                if (value)
                {
                    ...
                    auto const it =
                        std::find_if(std::cbegin(SortModes), std::cend(SortModes), test);
            ...
            case CustomVariantType::FilterModeType:
            {
                ...
                if (value)
                {
                    ...
                    // NOLINTNEXTLINE(readability-qualified-auto)
                    auto const it =
                        std::find_if(std::cbegin(FilterModes), std::cend(FilterModes), test);
            ...
            default: assert(false && "unhandled type"); break;
        }
    }
...
Prefs::~Prefs()
{
    ...
    for (int i = 0; i < PREFS_COUNT; ++i)
    {
        ...
        switch (Items[i].type)
        {
            case QVariant::Int: dictAdd(&current_settings, key, val.toInt()); break;
            ...
            case QVariant::String: dictAdd(&current_settings, key, val.toString()); break;

            case QVariant::Bool: dictAdd(&current_settings, key, val.toBool()); break;

            case QVariant::Double: dictAdd(&current_settings, key, val.toDouble()); break;
    ...
    if (!tr_variantFromFile(&file_settings, TR_VARIANT_FMT_JSON,
                            file.fileName().toUtf8().constData(), nullptr))
    {
...
void Prefs::initDefaults(tr_variant* d)
{
    ...
    auto constexpr SoundCommand = std::string_view {
