...
*/
  ...
  _registerFilter(op_text) {
    ...
    let text = op_text;
    for (const [op, c, a] of search_ops) {
      let op_keyword = op;
      if (op_keyword) {
        op_keyword = op_keyword.find((x) => op_text.startsWith(x));
        if (!op_keyword) {
          continue;
        }
        text = op_text.slice(op_keyword.length);
        this._filter.autocomplete = a;
        this._filter.controller = this._filter[c];
      }

      const { controller } = this._filter;
      if (controller) {
        let text_array = text.split(/,+/);
        const { autocomplete } = this._filter;
        if (autocomplete) {
          const new_ta = [];
          for (const t of text_array) {
            if (t) {
              new_ta.push(autocomplete.find((e) => e.startsWith(t)));
            }
          }
          text_array = new_ta;
        }
        ...
        if (op_keyword) {
          controller.push(text_array);
        } else {
          controller.at(-1).push(...text_array);
        }
