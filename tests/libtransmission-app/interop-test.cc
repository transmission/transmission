...
// The batch form keeps the usable arguments and drops the rest.
// Each dropped argument is reported on stderr, which no expectation here can see;
// tests/interop/delegation-test.sh checks the report with the real binaries.
TEST_F(InteropTest, encodesTheUsableArgumentsAndDropsTheRest)
{
    auto const magnet = std::string{ "magnet:?xt=urn:btih:00000000000000000000000000000000000000cc" };
    auto const args = std::vector<std::string>{ magnet, sandboxDir() + "/no-such-file.torrent" };

    EXPECT_EQ(std::vector<std::string>{ magnet }, interop::encode_metainfo_args(args));
}
...
// Probes the startup lock from outside while the coordinator publishes,
// so a test can see whether publication happened before the release.
...
public:
    ...
    GuardProbingTransport(GuardProbingTransport&&) = delete;
    GuardProbingTransport(GuardProbingTransport const&) = delete;
    GuardProbingTransport& operator=(GuardProbingTransport&&) = delete;
    GuardProbingTransport& operator=(GuardProbingTransport const&) = delete;

    void publish(interop::Instance& /*self*/) override
    {
        auto const filename = config_dir_ + '/' + std::string{ interop::StartupLockFilename };
...
TEST_F(InteropTest, publishesBeforeReleasingTheStartupLock)
{
    auto guard_was_held_during_publish = false;
    auto destroyed = false;
    auto script = Script{};
    auto instance = ScriptedInstance{ script };
    ...
    auto const filename = sandboxDir() + '/' + std::string{ interop::StartupLockFilename };
    auto prober = tr_lock_file{ filename };
    EXPECT_TRUE(prober.try_acquire());
}

TEST_F(InteropTest, destroysTransportWhenDestroyed)
{
    auto guard_was_held_during_publish = false;
    auto destroyed = false;
    auto script = Script{};
