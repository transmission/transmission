...
class RenameTest : public SessionTest
{
   ...
   protected:
    ...
    {
        auto path = makeString(
            tr_buildPath(top, "Felidae", "Felinae", "Acinonyx", "Cheetah", "Chester", nullptr));
        createFileWithContents(path, "It ain't easy bein' cheesy.\n");

        path = makeString(
            tr_buildPath(top, "Felidae", "Pantherinae", "Panthera", "Tiger", "Tony", nullptr));
        createFileWithContents(path, "They’re Grrrrreat!\n");

        path =
            makeString(tr_buildPath(top, "Felidae", "Felinae", "Felis", "catus", "Kyphi", nullptr));
        createFileWithContents(path, "Inquisitive\n");

        path = makeString(
            tr_buildPath(top, "Felidae", "Felinae", "Felis", "catus", "Saffron", nullptr));
    ...
    bool testFileExistsAndConsistsOfThisString(tr_torrent const* tor, tr_file_index_t file_index,
                                               std::string const& str)
    {
        ...
        if (path != nullptr)
        {
            ...
            success = contents != nullptr && str_len == contents_len
                      && memcmp(contents, str.data(), contents_len) == 0;
    ...
    int torrentRenameAndWait(tr_torrent* tor, char const* oldpath, char const* newname)
    {
        auto const on_rename_done =
            [](tr_torrent* /*tor*/, char const* /*oldpath*/, char const* /*newname*/, int error,
               void* user_data) noexcept { *static_cast<int*>(user_data) = error; };
...
TEST_F(RenameTest, singleFilenameTorrent)
{
    ...
    // this is a single-file torrent whose file is hello-world.txt, holding the string "hello,
    // world!"
    ...
    ***/
    ...
    EXPECT_FALSE(
        tr_sys_path_exists(tmpstr.c_str(), nullptr));  // confirm the old filename can't be found
    ...
    EXPECT_STREQ("foobar",
                 tor->info.files[0].name);  // confirm the file's name is now 'foobar' in our struct
    EXPECT_STREQ(nullptr,
                 strstr(tor->info.torrent,
                        "foobar"));  // confirm the name in the .torrent file hasn't changed
    tmpstr = makeString(tr_buildPath(tor->currentDir, "foobar", nullptr));
    EXPECT_TRUE(tr_sys_path_exists(
        tmpstr.c_str(), nullptr));  // confirm the file's name is now 'foobar' on the disk
    EXPECT_TRUE(testFileExistsAndConsistsOfThisString(
...
TEST_F(RenameTest, multifileTorrent)
{
    ...
    auto const expected_files = std::array<std::string, 4> {
        ...
        "Felidae/Felinae/Felis/catus/Saffron", "Felidae/Pantherinae/Panthera/Tiger/Tony"};
    ...
    ***/
    ...
    EXPECT_EQ(0,
              torrentRenameAndWait(tor, "Felidae/Pantherinae/Panthera/Snow Leopard/Tony", "10.6"));
