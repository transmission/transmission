 ...
 * **ip_endpoints_ipv4** String[] (default = ["https://ip4.transmissionbt.com/"]) A list of IP query endpoints to be tried in order, and the first valid result will be cached as the client's global IPv4 address. Specify an empty array to disable IP query for IPv4.
