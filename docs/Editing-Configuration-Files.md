 ...
 * **ip_endpoints_ipv4** String[] (default = ["https://ip4.transmissiontorrent.com/"]) A list of IP query endpoints to be tried in order, and the first valid result will be cached as the client's global IPv4 address. Specifying an empty array disables IP query for IPv4, but Transmission might not be able to announce your IPv4 address via legacy BEP-7 `&ipv4=` query parameter.
 * **ip_endpoints_ipv6** String[] (default = ["https://ip6.transmissiontorrent.com/"]) Same as `ip_endpoints_ipv4`, but for IPv6.
