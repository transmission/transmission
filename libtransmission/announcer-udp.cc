...
static void tau_tracker_timeout_reqs(struct tau_tracker* tracker)
{
    ...
    if (auto& reqs = tracker->announces; !std::empty(reqs))
    {
        for (auto it = std::begin(reqs); it != std::end(reqs);)
        {
            ...
            {
                logdbg(tracker->key, fmt::format("timeout announce req {}", fmt::ptr(&req)));
                req.fail(false, true, "");
                it = reqs.erase(it);
            }
            else
    ...
    if (auto& reqs = tracker->scrapes; !std::empty(reqs))
    {
        for (auto it = std::begin(reqs); it != std::end(reqs);)
        {
            ...
            if (cancel_all || req.created_at + TauRequestTtl < now)
            {
                logdbg(tracker->key, fmt::format("timeout scrape req {}", fmt::ptr(&req)));
                req.fail(false, true, "");
                it = reqs.erase(it);
            }
            else
...
static void tau_tracker_upkeep_ex(struct tau_tracker* tracker, bool timeout_reqs)
{
    ...
    if (tracker->addr != nullptr && tracker->connection_expiration_time <= now && tracker->connecting_at == 0)
    {
        ...
        logdbg(tracker->key, fmt::format("Trying to connect. Transaction ID is {}", tracker->connection_transaction_id));
...
bool tau_handle_message(tr_session* session, uint8_t const* msg, size_t msglen)
{
    ...
    for (auto& tracker : tau->trackers)
    {
        ...
        {
            logdbg(tracker.key, fmt::format("{} is my connection request!", transaction_id));
        ...
        if (auto& reqs = tracker.announces; !std::empty(reqs))
        {
            ...
            if (it != std::end(reqs))
            {
                logdbg(tracker.key, fmt::format("{} is an announce request!", transaction_id));
                auto req = *it;
                it = reqs.erase(it);
        ...
        if (auto& reqs = tracker.scrapes; !std::empty(reqs))
        {
            ...
            if (it != std::end(reqs))
            {
                logdbg(tracker.key, fmt::format("{} is a scrape request!", transaction_id));
                auto req = *it;
                it = reqs.erase(it);
                req.onResponse(action_id, buf);
                evbuffer_free(buf);
