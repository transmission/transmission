...
// Helper to make socket reuseable
bool make_socket_reuseable(tr_socket_t sock)
{
    int const optval = 1;
#ifdef SO_REUSEADDR
    if (setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, reinterpret_cast<char const*>(&optval), sizeof(optval)) < 0)
    {
        return false;
    }
#endif
#ifdef SO_REUSEPORT
    if (setsockopt(sock, SOL_SOCKET, SO_REUSEPORT, reinterpret_cast<char const*>(&optval), sizeof(optval)) < 0)
    {
        return false;
    }
#endif
...
bool make_socket_nonblocking(tr_socket_t sock)
{
#ifdef _WIN32
    u_long mode = 1;
...
// Helper to make IPv6 socket IPv6-only
bool make_socket_ipv6only(tr_socket_t sock)
{
#ifdef IPV6_V6ONLY
    int const optval = 1;
    return setsockopt(sock, IPPROTO_IPV6, IPV6_V6ONLY, reinterpret_cast<char const*>(&optval), sizeof(optval)) == 0;
#else
    return true;
#endif
}
...
void tr_session::tr_udp_core_libuv::on_udp_readable(struct uv_poll_s* handle, int status, int events)
{
    if (status < 0)
    {
    ...
    auto* const self = static_cast<tr_udp_core_libuv*>(handle->data);
    TR_ASSERT(self != nullptr);

    // Determine which socket triggered the event
    tr_socket_t s = TR_BAD_SOCKET;
    if (handle == self->udp4_poll_)
    {
        s = self->udp4_socket_;
    }
    else if (handle == self->udp6_poll_)
    {
        s = self->udp6_socket_;
    }
    ...
    auto* const session = &self->session_;
...
tr_session::tr_udp_core_libuv::tr_udp_core_libuv(tr_session& session, tr_port udp_port)
    : udp_port_{ udp_port }
    , session_{ session }
{
    ...
    if (auto sock = socket(PF_INET, SOCK_DGRAM, 0); sock != TR_BAD_SOCKET)
    {
        ...
        else
        {
            ...
            // Create and start uv_poll for IPv4 socket
            udp4_poll_ = new uv_poll_t{};
            uv_poll_init_socket(session.uv_loop(), udp4_poll_, udp4_socket_);
            udp4_poll_->data = this;
            uv_poll_start(udp4_poll_, UV_READABLE, &tr_udp_core_libuv::on_udp_readable);
        }
    }
    ...
    {
        (void)make_socket_reuseable(sock);
        (void)make_socket_ipv6only(sock);
        ...
        else
        {
            ...
            // Create and start uv_poll for IPv6 socket
            udp6_poll_ = new uv_poll_t{};
            uv_poll_init_socket(session.uv_loop(), udp6_poll_, udp6_socket_);
            udp6_poll_->data = this;
            uv_poll_start(udp6_poll_, UV_READABLE, &tr_udp_core_libuv::on_udp_readable);
        }
    }
}

tr_session::tr_udp_core_libuv::~tr_udp_core_libuv()
{
    if (udp6_poll_ != nullptr)
    {
        uv_poll_stop(udp6_poll_);
        uv_close(
            reinterpret_cast<uv_handle_t*>(udp6_poll_),
    ...
    if (udp4_poll_ != nullptr)
    {
        uv_poll_stop(udp4_poll_);
        uv_close(
            reinterpret_cast<uv_handle_t*>(udp4_poll_),
            [](uv_handle_t* handle) { delete reinterpret_cast<uv_poll_t*>(handle); });
        udp4_poll_ = nullptr;
    }
...
void tr_session::tr_udp_core_libuv::sendto(void const* buf, size_t buflen, struct sockaddr const* to, socklen_t const tolen)
    const
{
