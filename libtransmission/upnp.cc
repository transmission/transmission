...
tr_port_forwarding tr_upnpPulse(tr_upnp* handle, tr_port port, bool isEnabled, bool doPortCheck, char const* bindaddr)
{
        ...
        isFutureReady(*handle->discover_future))
    {
        ...
        {
            tr_log::info::add(TR_LOC, fmt::format("Found Internet Gateway Device '{}'", handle->urls.controlURL), Key);
            tr_log::info::add(TR_LOC, fmt::format("Local address is '{}'", handle->lanaddr), Key);
    ...
    if (handle->state == UpnpState::WILL_MAP)
    {
        ...
        if (handle->isMapped)
        {
            tr_log::info::add(TR_LOC, "Port forwarding successful!", Key);
            handle->port = port;
            handle->state = UpnpState::IDLE;
        }
        else
