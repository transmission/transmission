...
// Helper to make socket non-blocking for libuv
bool makeSocketNonBlocking(tr_socket_t sock)
{
#ifdef _WIN32
    u_long mode = 1;
...
class tr_lpd_libuv_impl final : public tr_lpd
{
public:
    tr_lpd_libuv_impl(Mediator& mediator, struct uv_loop_s* uv_loop)
        : mediator_{ mediator }
        , uv_loop_{ uv_loop }
    ...
    tr_lpd_libuv_impl(tr_lpd_libuv_impl&&) = delete;
    tr_lpd_libuv_impl(tr_lpd_libuv_impl const&) = delete;
    tr_lpd_libuv_impl& operator=(tr_lpd_libuv_impl&&) = delete;
    tr_lpd_libuv_impl& operator=(tr_lpd_libuv_impl const&) = delete;

    ~tr_lpd_libuv_impl() override
    {
        for (auto* poll : polls_)
        {
            if (poll != nullptr)
            {
                uv_poll_stop(poll);
                uv_close(
                    reinterpret_cast<uv_handle_t*>(poll),
...
private:
    ...
    bool initImpl()
    {
        ...
        if constexpr (ip_protocol == TR_AF_INET6)
        {
            ...
            if (setsockopt(sock, IPPROTO_IPV6, IPV6_V6ONLY, reinterpret_cast<char const*>(&opt_on), sizeof(opt_on)) == -1)
            {
                return false;
            }
#endif
        ...
        // Initialize uv_poll for this socket
        auto* poll = new uv_poll_t;
        uv_poll_init_socket(uv_loop_, poll, sock);
        poll->data = this;
        polls_[ip_protocol] = poll;

        uv_poll_start(poll, UV_READABLE, poll_callback<ip_protocol>);
    ...
    * @see DoS */
    ...
    static void poll_callback(uv_poll_t* handle, int status, int events)
    {
        if (status < 0)
        {
            tr_logAddError(fmt::format("LPD poll error: {}", uv_strerror(status)));
            return;
        }

        if ((events & UV_READABLE) != 0)
        {
            auto* self = static_cast<tr_lpd_libuv_impl*>(handle->data);
            self->onCanRead(ip_protocol);
        }
    }
    ...
    std::array<uv_poll_t*, NUM_TR_AF_INET_TYPES> polls_ = { nullptr, nullptr };
...
std::unique_ptr<tr_lpd> tr_lpd::create_libuv(Mediator& mediator, struct uv_loop_s* uv_loop)
{
    return std::make_unique<tr_lpd_libuv_impl>(mediator, uv_loop);
}
