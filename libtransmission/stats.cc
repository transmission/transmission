...
tr_session_stats tr_stats::load_old_stats(std::string_view config_dir)
{
    ...
    ret.downloadedBytes = map->value_if<int64_t>(TR_KEY_downloaded_bytes).value_or(0);
    ret.filesAdded = map->value_if<int64_t>(TR_KEY_files_added).value_or(0);
    ret.secondsActive = map->value_if<int64_t>(TR_KEY_seconds_active).value_or(0);
    ret.sessionCount = map->value_if<int64_t>(TR_KEY_session_count).value_or(0);
    ret.uploadedBytes = map->value_if<int64_t>(TR_KEY_uploaded_bytes).value_or(0);
    return ret;
}
...
void tr_stats::save() const
{
    ...
    var = api_compat::convert(var, api_compat::get_export_settings_style());
