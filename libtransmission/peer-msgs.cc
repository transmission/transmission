...
#define logtrace(peer_msgs, msg) \
    do \
    { \
        if (tr_log::trace::enabled()) \
        { \
            tr_log::trace::add(TR_LOC, msg, tr_torrentName(msgs->torrent)); \
        } \
    } while (0)
...
static void pokeBatchPeriod(tr_peerMsgsImpl* msgs, int interval)
{
    if (msgs->outMessagesBatchPeriod > interval)
    {
        ...
        logtrace(msgs, fmt::format("lowering batch interval to {} seconds", interval));
    }
}

static void dbgOutMessageLen(tr_peerMsgsImpl* msgs)
{
    logtrace(msgs, fmt::format("outMessage size is now {}", evbuffer_get_length(msgs->outMessages)));
}
...
static void protocolSendReject(tr_peerMsgsImpl* msgs, struct peer_request const* req)
{
    ...
    logtrace(msgs, fmt::format("rejecting {}:{}->{}...", req->index, req->offset, req->length));
...
static void protocolSendRequest(tr_peerMsgsImpl* msgs, struct peer_request const& req)
{
    ...
    logtrace(msgs, fmt::format("requesting {}:{}->{}...", req.index, req.offset, req.length));
    dbgOutMessageLen(msgs);
...
static void protocolSendCancel(tr_peerMsgsImpl* msgs, peer_request const& req)
{
    ...
    logtrace(msgs, fmt::format("cancelling {}:{}->{}...", req.index, req.offset, req.length));
    dbgOutMessageLen(msgs);
    pokeBatchPeriod(msgs, ImmediatePriorityIntervalSecs);
}
...
static void protocolSendPort(tr_peerMsgsImpl* msgs, uint16_t port)
{
    ...
    logtrace(msgs, fmt::format("sending Port {}", port));
...
static void protocolSendHave(tr_peerMsgsImpl* msgs, tr_piece_index_t index)
{
    ...
    logtrace(msgs, fmt::format("sending Have {}", index));
    dbgOutMessageLen(msgs);
...
static void protocolSendChoke(tr_peerMsgsImpl* msgs, bool choke)
{
    ...
    logtrace(msgs, choke ? "sending choke" : "sending unchoke");
    dbgOutMessageLen(msgs);
    pokeBatchPeriod(msgs, ImmediatePriorityIntervalSecs);
}
...
static void parseLtepHandshake(tr_peerMsgsImpl* msgs, uint32_t len, struct evbuffer* inbuf)
{
    ...
    if (len <= 4096)
    {
        logtrace(msgs, fmt::format("here is the handshake: [%*.*s]", TR_ARG_TUPLE(int(len), int(len), tmp)));
    }
    else
    {
        logtrace(msgs, fmt::format("handshake length is too big ({}), printing skipped", len));
    }
    ...
    if (tr_variant* sub = nullptr; tr_variantDictFindDict(&val, TR_KEY_m, &sub))
    {
        if (tr_variantDictFindInt(sub, TR_KEY_ut_pex, &i))
        {
            ...
            logtrace(msgs, fmt::format("msgs->ut_pex is {}", int(msgs->ut_pex_id)));
        }
        ...
        if (tr_variantDictFindInt(sub, TR_KEY_ut_metadata, &i))
        {
            ...
            logtrace(msgs, fmt::format("msgs->ut_metadata_id is {}", int(msgs->ut_metadata_id)));
        }
    ...
    if (tr_variantDictFindInt(&val, TR_KEY_p, &i))
    {
        ...
        logtrace(msgs, fmt::format("peer's port is now {}", int(i)));
    }
...
static void parseUtMetadata(tr_peerMsgsImpl* msgs, uint32_t msglen, struct evbuffer* inbuf)
{
    ...
    logtrace(
        msgs,
        fmt::format("got ut_metadata msg: type {}, piece {}, total_size {}", int(msg_type), int(piece), int(total_size)));
...
static void parseLtep(tr_peerMsgsImpl* msgs, uint32_t msglen, struct evbuffer* inbuf)
{
    ...
    {
        logtrace(msgs, fmt::format("skipping unknown ltep message ({})", int(ltep_msgid)));
...
static ReadState readBtId(tr_peerMsgsImpl* msgs, struct evbuffer* inbuf, size_t inlen)
{
    ...
    msgs->incoming.id = id;
    logtrace(msgs, fmt::format("msgs->incoming.id is now {}; msgs->incoming.length is {}", id, (size_t)msgs->incoming.length));
...
static ReadState readBtPiece(tr_peerMsgsImpl* msgs, struct evbuffer* inbuf, size_t inlen, size_t* setme_piece_bytes_read)
{
    ...
    if (req->length == 0)
    {
        ...
        logtrace(msgs, fmt::format("got incoming block header {}:{}->{}", req->index, req->offset, req->length));
        return READ_NOW;
    }
...
static ReadState readBtMessage(tr_peerMsgsImpl* msgs, struct evbuffer* inbuf, size_t inlen)
{
    ...
    logtrace(msgs, fmt::format("got BT id {}, len {}, buffer size is {}", int(id), int(msglen), inlen));
    ...
    {
        logdbg(msgs, fmt::format("bad packet - BT message #{} with a length of {}", int(id), int(msglen)));
    ...
    case BtPeerMsgs::Have:
        ...
        logtrace(msgs, fmt::format("got Have: {}", ui32));
    ...
    case BtPeerMsgs::Request:
        {
            ...
            logtrace(msgs, fmt::format("got Request: {}:{}->{}", r.index, r.offset, r.length));
    ...
    case BtPeerMsgs::Cancel:
        {
            ...
            logtrace(msgs, fmt::format("got a Cancel {}:{}->{}", r.index, r.offset, r.length));
    ...
    default:
        logtrace(msgs, fmt::format("peer sent us an UNKNOWN: {}", int(id)));
...
static int clientGotBlock(tr_peerMsgsImpl* msgs, struct evbuffer* data, struct peer_request const* req)
{
    ...
    if (!requestIsValid(msgs, req))
    {
        logdbg(msgs, fmt::format("dropping invalid block {}:{}->{}", req->index, req->offset, req->length));
        return EBADMSG;
    }

    if (req->length != msgs->torrent->blockSize(block))
    {
        logdbg(msgs, fmt::format("wrong block size -- expected {}, got {}", msgs->torrent->blockSize(block), req->length));
        return EMSGSIZE;
    }

    logtrace(msgs, fmt::format("got block {}:{}->{}", req->index, req->offset, req->length));
...
static ReadState canRead(tr_peerIo* io, void* vmsgs, size_t* piece)
{
    ...
    logtrace(msgs, fmt::format("canRead: inlen is {}, msgs->state is {}", inlen, int(msgs->state)));
    ...
    logtrace(msgs, fmt::format("canRead: ret is {}", int(ret)));
...
static void updateMetadataRequests(tr_peerMsgsImpl* msgs, time_t now)
{
    ...
    if (msgs->peerSupportsMetadataXfer && tr_torrentGetNextMetadataRequest(msgs->torrent, now, &piece))
    {
        ...
        logtrace(msgs, fmt::format("requesting metadata piece #{}", piece));
...
static size_t fillOutputBuffer(tr_peerMsgsImpl* msgs, time_t now)
{
    ...
    **/
    ...
    {
        logtrace(msgs, fmt::format("started an outMessages batch (length is {})", evbuffer_get_length(msgs->outMessages)));
    ...
    else if (haveMessages && now - msgs->outMessagesBatchedAt >= msgs->outMessagesBatchPeriod)
    {
        ...
        logtrace(msgs, fmt::format("flushing outMessages... to {} (length is {})", (void*)msgs->io, len));
    ...
    if (tr_peerIoGetWriteBufferSpace(msgs->io, now) >= tr_block_info::BlockSize && popNextRequest(msgs, &req))
    {
        ...
        if (requestIsValid(msgs, &req) && msgs->torrent->hasPiece(req.index))
        {
            ...
            else
            {
                ...
                logtrace(msgs, fmt::format("sending block {}:{}->{}", req.index, req.offset, req.length));
                TR_ASSERT(n == msglen);
...
static void gotError(tr_peerIo* /*io*/, short what, void* vmsgs)
{
    ...
    {
        logdbg(msgs, fmt::format("libevent got a timeout, what={}", what));
    }

    if ((what & (BEV_EVENT_EOF | BEV_EVENT_ERROR)) != 0)
    {
        logdbg(msgs, fmt::format("libevent got an error! what={}, errno={} ({})", what, errno, tr_strerror(errno)));
    }
...
static void sendBitfield(tr_peerMsgsImpl* msgs)
{
    ...
    logtrace(msgs, fmt::format("sending bitfield... outMessage size is now {}", evbuffer_get_length(out)));
    pokeBatchPeriod(msgs, ImmediatePriorityIntervalSecs);
}
...
static void sendPex(tr_peerMsgsImpl* msgs)
{
    if (msgs->peerSupportsPex && msgs->torrent->allowsPex())
    {
        ...
        logtrace(
            msgs,
            fmt::format(
                "pex: old peer count {}+{}, new peer count {}+{}, added {}+{}, removed {}+{}",
                msgs->pexCount,
                msgs->pexCount6,
                newCount,
                newCount6,
        ...
        else
        {
            ...
            evbuffer_add_buffer(out, payload);
            pokeBatchPeriod(msgs, HighPriorityIntervalSecs);
            logtrace(msgs, fmt::format("sending a pex message; outMessage size is now {}", evbuffer_get_length(out)));
            dbgOutMessageLen(msgs);
