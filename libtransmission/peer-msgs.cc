...
public:
        ...
        , callback_data_{ callback_data }
    {
        ...
        if (io->supports_utp())
        {
            tr_peerMgrSetUtpSupported(torrent, io->address(), io->port());
            tr_peerMgrSetUtpFailed(torrent, io->address(), io->port(), false);
        }
...
void parseLtepHandshake(tr_peerMsgsImpl* msgs, uint32_t len)
{
    ...
    if (tr_variant* sub = nullptr; tr_variantDictFindDict(&val, TR_KEY_m, &sub))
    {
        ...
        if (tr_variantDictFindInt(sub, TR_KEY_ut_holepunch, &i))
        {
            ...
            tr_peerMgrSetUtpFailed(msgs->torrent, msgs->io->address(), msgs->io->port(), false);
        }
    }
