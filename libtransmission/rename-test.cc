...
protected:
    ...
    void torrentRemoveAndWait(tr_torrent* tor, int expected_torrent_count) {
        ...
        while (tr_sessionCountTorrents(session_) != expected_torrent_count) {
            tr_wait_msec(10);
        }
    }
    ...
    tr_torrent* create_torrent_from_base64_metainfo(tr_ctor* ctor, char const* metainfo_base64) {
        // create the torrent ctor
        size_t metainfo_len;
        auto* metainfo = reinterpret_cast<char*>(tr_base64_decode_str(metainfo_base64, &metainfo_len));
    ...
    bool testFileExistsAndConsistsOfThisString(tr_torrent const* tor, tr_file_index_t fileIndex, char const* str) {
        size_t const str_len = strlen(str);
        bool success = false;

        auto* path = tr_torrentFindFile(tor, fileIndex);
        if (path != nullptr) {
            ...
            success = contents != nullptr && str_len == contents_len && memcmp(contents, str, contents_len) == 0;
...
#define EXPECT_HAVE_NONE(tor, totalSize) \
    do \
    { \
        auto const* tst = tr_torrentStat(tor); \
        EXPECT_EQ(TR_STATUS_STOPPED, tst->activity); \
        EXPECT_EQ(TR_STAT_OK, tst->error); \
        EXPECT_EQ(totalSize, tst->sizeWhenDone); \
        EXPECT_EQ(totalSize, tst->leftUntilDone); \
...
TEST_F(RenameTest, single_filename_torrent)
{
    uint64_t loaded;
    ...
    auto* tor = create_torrent_from_base64_metainfo(ctor,
    ...
    EXPECT_HAVE_NONE(tor, totalSize);
    ...
    ***/
    ...
    auto tmpstr = make_string(tr_buildPath(tor->currentDir, "hello-world.txt", nullptr));
    EXPECT_TRUE(tr_sys_path_exists(std::data(tmpstr), nullptr));
    EXPECT_STREQ("hello-world.txt", tr_torrentName(tor));
    EXPECT_EQ(0, torrentRenameAndWait(tor, tor->info.name, "foobar"));
    EXPECT_FALSE(tr_sys_path_exists(std::data(tmpstr), nullptr)); /* confirm the old filename can't be found */
    ...
    tmpstr = make_string(tr_buildPath(tor->currentDir, "foobar", nullptr));
    EXPECT_TRUE(tr_sys_path_exists(std::data(tmpstr), nullptr)); /* confirm the file's name is now 'foobar' on the disk */
    ...
    ***/
    ...
    EXPECT_FALSE(tr_sys_path_exists(std::data(tmpstr), nullptr));
...
TEST_F(RenameTest, multifile_torrent)
{
    uint64_t loaded;
    tr_torrent* tor;
    tr_ctor* ctor;
    ...
    EXPECT_EQ(totalSize, tor->info.totalSize);
    EXPECT_EQ(4, tor->info.fileCount);

    for (tr_file_index_t i = 0; i < 4; ++i)
    {
        EXPECT_STREQ(expected_files[i], files[i].name);
    }
    ...
    **/
    ...
    EXPECT_STREQ(expected_files[0], files[0].name);
    ...
    EXPECT_STREQ(expected_files[3], files[3].name);
    ...
    loaded = tr_torrentLoadResume(tor, ~0, ctor, nullptr);
    EXPECT_NE(0, (loaded & TR_FR_FILENAMES));
    EXPECT_STREQ(expected_files[0], files[0].name);
    EXPECT_STREQ("Felidae/Felinae/Felis/placeholder/Kyphi", files[1].name);
    EXPECT_STREQ("Felidae/Felinae/Felis/placeholder/Saffron", files[2].name);
    EXPECT_STREQ(expected_files[3], files[3].name);
    ...
    ***/
    ...
    EXPECT_NE(nullptr, str);
    tr_sys_path_remove(str, nullptr);
    tmp = tr_sys_path_dirname(str, nullptr);
...
TEST_F(RenameTest, partial_file)
{
