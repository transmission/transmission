    ...
    while (!stack.empty())
    {
        ...
        if (v != nullptr)
        {
            ...
            default:
                ...
                tr_log::error::add(TR_LOC, "Invalid metadata");
                break;
            }
        }
    }
...
static void tr_variantListCopy(tr_variant* target, tr_variant const* src)
{
    ...
    while ((val = tr_variantListChild(const_cast<tr_variant*>(src), i)) != nullptr)
    {
        ...
        {
            tr_log::error::add(TR_LOC, "tr_variantListCopy skipping item");
        }
...
int tr_variantToFile(tr_variant const* v, tr_variant_fmt fmt, std::string const& filename)
{
    ...
    if (error != nullptr)
    {
        tr_log::error::add(TR_LOC, fmt::format("Error saving '{filename}': {message} ({code})",
