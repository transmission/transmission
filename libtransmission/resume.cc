...
tr_variant bitfield_to_raw(tr_bitfield const& b)
{
    ...
    return std::string_view{ reinterpret_cast<char const*>(std::data(raw)), std::size(raw) };
}
...
tr_resume::fields_t load_from_file(tr_torrent* tor, tr_torrent::ResumeHelper& helper, tr_resume::fields_t fields_to_load)
{
    ...
    if (auto i = map.value_if<int64_t>({ TR_KEY_max_peers, TR_KEY_max_peers_kebab });
        i && (fields_to_load & tr_resume::MaxPeers) != 0)
    {
    ...
    if (auto i = map.value_if<int64_t>({ TR_KEY_added_date, TR_KEY_added_date_kebab });
        i && (fields_to_load & tr_resume::AddedDate) != 0)
    {
        helper.load_date_added(static_cast<time_t>(*i));
        fields_loaded |= tr_resume::AddedDate;
    }

    if (auto i = map.value_if<int64_t>({ TR_KEY_done_date, TR_KEY_done_date_kebab });
        i && (fields_to_load & tr_resume::DoneDate) != 0)
    {
        helper.load_date_done(static_cast<time_t>(*i));
        fields_loaded |= tr_resume::DoneDate;
    }

    if (auto i = map.value_if<int64_t>({ TR_KEY_activity_date, TR_KEY_activity_date_kebab });
        i && (fields_to_load & tr_resume::ActivityDate) != 0)
    {
        tor->set_date_active(*i);
        fields_loaded |= tr_resume::ActivityDate;
    }

    if (auto i = map.value_if<int64_t>({ TR_KEY_seeding_time_seconds, TR_KEY_seeding_time_seconds_kebab });
        i && (fields_to_load & tr_resume::TimeSeeding) != 0)
    {
        helper.load_seconds_seeding_before_current_start(*i);
        fields_loaded |= tr_resume::TimeSeeding;
    }

    if (auto i = map.value_if<int64_t>({ TR_KEY_downloading_time_seconds, TR_KEY_downloading_time_seconds_kebab });
