...
/**
 * @brief Rich Salz's classic implementation of shell-style pattern matching for
 * ?, \, [], and * characters.
 ...
 */
uint8_t* tr_loadFile(char const* filename, size_t* size, struct tr_error** error) TR_GNUC_MALLOC
    TR_GNUC_NONNULL(1);
...
/**
 * @brief Convenience wrapper around timer_add() to have a timer wake up in a
...
/**
 * @brief make a copy of 'str' whose non-utf8 content has been corrected or
 * stripped
 * @return a newly-allocated string that must be freed with tr_free()
 * @param str the string to make a clean copy of
 * @param len the length of the string to copy. If -1, the entire string is
 * used.
 */
...
#define tr_main(...)                                                                   \
    ...
    int main(int argc, char* argv[]) { return tr_main_win32(argc, argv, &main_impl); } \
...
/** @brief Free pointers in a NULL-terminated array (the array itself is not
 * freed) */
 ...
 * @param in is a void* so that callers can pass in both signed & unsigned
 * without a cast
 * @param len length of the substring to copy. if a length less than zero is
 * passed in, strlen(len) is used
...
int tr_lowerBound(void const* key, void const* base, size_t nmemb, size_t size,
                  tr_voidptr_compare_func compar, bool* exact_match) TR_GNUC_HOT
...
/**
 * @brief sprintf() a string into a newly-allocated buffer large enough to hold
 * it
...
/** @brief Portability wrapper for strlcpy() that uses the system implementation
 * if available */
size_t tr_strlcpy(void* dst, void const* src, size_t siz);

/** @brief Portability wrapper for snprintf() that uses the system
 * implementation if available */
int tr_snprintf(void* buf, size_t buflen, char const* fmt, ...) TR_GNUC_PRINTF(3, 4)
    TR_GNUC_NONNULL(1, 3);
...
/** @brief Returns true if the string ends with the specified case-insensitive
 * suffix */
bool tr_str_has_suffix(char const* str, char const* suffix);

/** @brief Portability wrapper for memmem() that uses the system implementation
 * if available */
char const* tr_memmem(char const* haystack, size_t haystack_len, char const* needle,
                      size_t needle_len);

/** @brief Portability wrapper for strcasestr() that uses the system
 * implementation if available */
char const* tr_strcasestr(char const* haystack, char const* needle);

/** @brief Portability wrapper for strsep() that uses the system implementation
 * if available */
...
/** @brief convenience function to determine if an address is an IP address
 * (IPv4 or IPv6) */
bool tr_addressIsIP(char const* address);

/** @brief return true if the url is a http or https or UDP url that
 * Transmission understands */
bool tr_urlIsValidTracker(char const* url);

/** @brief return true if the url is a [ http, https, ftp, sftp ] url that
 * Transmission understands */
...
bool tr_urlParse(char const* url, size_t url_len, char** setme_scheme, char** setme_host,
                 int* setme_port, char** setme_path) TR_GNUC_NONNULL(1);
 ...
 * @return a newly-allocated array of integers that must be freed with
 * tr_free(), or NULL if a fragment of the string can't be parsed.
 *
 * For example, "5-8" will return [ 5, 6, 7, 8 ] and setmeCount will be 4.
 */
int* tr_parseNumberRange(char const* str, size_t str_len, int* setmeCount) TR_GNUC_MALLOC
    TR_GNUC_NONNULL(1);
...
/** @brief Portability wrapper for localtime_r() that uses the system
 * implementation if available */
struct tm* tr_localtime_r(time_t const* _clock, struct tm* _result);

/** @brief Portability wrapper for gmtime_r() that uses the system
 * implementation if available */
 ...
 */
bool tr_moveFile(char const* oldpath, char const* newpath, struct tr_error** error)
    TR_GNUC_NONNULL(1, 2);
...
/** @brief Private libtransmission variable that's visible only for inlining in
 * tr_time() */
...
/** @brief Portability wrapper for htonll() that uses the system implementation
 * if available */
...
/* example: tr_formatter_size_init(1024, _("KiB"), _("MiB"), _("GiB"),
 * _("TiB")); */
...
/** @brief Get environment variable value as string (should be freed
 * afterwards). */
