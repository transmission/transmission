    ...
    , session_{ session }
{
    ...
    if (auto sock = socket(PF_INET, SOCK_DGRAM, 0); sock != TR_BAD_SOCKET)
    {
        ...
        else
        {
            ...
            udp4_socket_ = sock;
            udp4_event_.reset(libtransmission::evhelpers::event_new_pri2(session_.event_base(), udp4_socket_, EV_READ | EV_PERSIST, event_callback, &session_));
    ...
    else if (auto sock = socket(PF_INET6, SOCK_DGRAM, 0); sock != TR_BAD_SOCKET)
    {
        ...
        else
        {
            ...
            set_socket_buffers(sock, session_.allowsUTP());
            udp6_socket_ = sock;
            udp6_event_.reset(libtransmission::evhelpers::event_new_pri2(session_.event_base(), udp6_socket_, EV_READ | EV_PERSIST, event_callback, &session_));
