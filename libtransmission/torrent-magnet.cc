...
void tr_torrentMagnetDoIdleWork(tr_torrent* const tor)
{
    ...
    if (auto* const m = tor->incomplete_metadata; m != nullptr && std::empty(m->pieces_needed))
    {
...
void tr_torrentSetMetadataPiece(tr_torrent* tor, int piece, void const* data, size_t len)
{
    ...
    tr_incomplete_metadata* const m = tor->incomplete_metadata;
    if (m == nullptr)
    {
        return;
    }
...
std::optional<int> tr_torrentGetNextMetadataRequest(tr_torrent* tor, time_t now)
{
    ...
    struct tr_incomplete_metadata* m = tor->incomplete_metadata;
    if (m == nullptr)
    {
        return {};
    }
...
double tr_torrentGetMetadataPercent(tr_torrent const* tor)
{
    ...
    auto const* const m = tor->incomplete_metadata;
