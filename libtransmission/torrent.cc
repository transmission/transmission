...
bool tr_torrent::sameSize(tr_file_index_t file_index) {
    std::optional<tr_torrent_files::FoundFile> foundFile = findFile(file_index);
    if (foundFile) {
        auto const file_length = fileSize(file_index);
        return foundFile->filesize() == file_length;
    }
    return false;
}
...
bool tr_torrent::setTrackerList(std::string_view text) {
    ...
    if (this->error == TR_STAT_TRACKER_WARNING || this->error == TR_STAT_TRACKER_ERROR) {
        ...
        if (std::any_of(
                ...
                [error_url](auto const &tracker) {
                    return tracker.announce == error_url;
                })) {
