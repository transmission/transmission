...
public:
        ...
        , sock_{ sock }
    {
        ...
        event_read_.reset(tr::evhelpers::event_new_pri2(session.event_base(), sock_, EV_READ, event_read_cb, this));
        event_write_.reset(tr::evhelpers::event_new_pri2(session.event_base(), sock_, EV_WRITE, event_write_cb, this));
...
private:
    ...
    size_t try_write_impl(OutBuf& buf, size_t n_bytes, tr_error* error) override
    {
        ...
        if (auto const n_sent = send(sock_, reinterpret_cast<char const*>(std::data(buf)), n_bytes, 0); n_sent >= 0)
        {
            buf.drain(n_sent);
            return n_sent;
        }
