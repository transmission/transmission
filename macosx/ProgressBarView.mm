...
/// Layers
/// Ordered by z-index (from back to front).
...
- (void)setupLayers
{
    ...
    self.piecesBar.actions = actions;
...
{
    self.ratioLayer.colors = [[ModernProgressGradients progressLightGrayGradient] valueForKey:@"cgColorKVC"];
    self.wantedLayer.colors = [[ModernProgressGradients progressWhiteGradient] valueForKey:@"cgColorKVC"];
    self.unavailableLayer.colors = [[ModernProgressGradients progressRedGradient] valueForKey:@"cgColorKVC"];

    self.unwantedLayer.locations = [ModernProgressGradients locations];
    self.unavailableLayer.locations = [ModernProgressGradients locations];
    self.wantedLayer.locations = [ModernProgressGradients locations];
    self.haveLayer.locations = [ModernProgressGradients locations];
    self.ratioLayer.locations = [ModernProgressGradients locations];
}
...
- (void)updateRegularBarInRect:(CGRect)rect forTorrent:(Torrent*)torrent
{
    ...
    if (!torrent.allDownloaded)
    {
        ...
        self.wantedLayer.frame = CGRectMake(currentX, 0, availableWidth, rect.size.height);
        currentX += availableWidth;

        self.unavailableLayer.frame = CGRectMake(currentX, 0, unavailableWidth, rect.size.height);
        currentX += unavailableWidth;
    }
    else
    {
        self.wantedLayer.frame = CGRectZero;
        self.unavailableLayer.frame = CGRectZero;
    }
    ...
    if (remainingWidth > 0)
    {
        self.unwantedLayer.frame = CGRectMake(currentX, 0, remainingWidth, rect.size.height);
    }
    else
    {
        self.unwantedLayer.frame = CGRectZero;
    }
...
{
    self.haveLayer.colors = [[self haveLayerColorsForTorrent:torrent] valueForKey:@"cgColorKVC"];
    self.unwantedLayer.colors = [[self unwantedLayerColorsForTorrent:torrent] valueForKey:@"cgColorKVC"];
}

- (void)updatePiecesContentForTorrent:(Torrent*)torrent
{
    // Copy method from previous bar
    // Fill a solid color bar for magnet links
    if (torrent.magnet)
    {
        __auto_type alpha = [self.fDefaults boolForKey:@"SmallView"] ? 0.25 : 1.0;
        self.piecesBar.backgroundColor = NSApp.darkMode ? NSColor.controlColor.CGColor : [NSColor colorWithCalibratedWhite:1.0 alpha:alpha].CGColor;
        self.piecesBar.contents = nil;
        return;
    }
    ...
    torrent.previousFinishedPieces = finishedIndexes.count > 0 ? finishedIndexes : nil; //don't bother saving if none are complete
    self.piecesBar.contents = (id)[bitmap CGImage];
}
