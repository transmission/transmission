...
static void dialogResponse(GtkDialog* dialog, gint response, gpointer gdata)
{
    ...
    if (response == TR_RESPONSE_RESET)
    {
        ...
        GtkWidget* w = gtk_message_dialog_new(GTK_WINDOW(dialog), flags, GTK_MESSAGE_QUESTION,
                                              GTK_BUTTONS_NONE, "%s", primary);
        gtk_dialog_add_buttons(GTK_DIALOG(w), _("_Cancel"), GTK_RESPONSE_CANCEL, _("_Reset"),
                               TR_RESPONSE_RESET, NULL);
...
GtkWidget* gtr_stats_dialog_new(GtkWindow* parent, TrCore* core)
{
    ...
    d = gtk_dialog_new_with_buttons(_("Statistics"), parent, GTK_DIALOG_DESTROY_WITH_PARENT,
                                    _("_Reset"), TR_RESPONSE_RESET, _("_Close"), GTK_RESPONSE_CLOSE,
                                    NULL);
