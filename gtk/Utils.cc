...
std::string tr_format_future_time(time_t seconds)
{
    ...
    if (auto const seconds_from_now = seconds % 60; seconds_from_now > 0)
    {
        return fmt::format(
            fmt::runtime(
...
std::string tr_format_past_time(time_t seconds)
{
    ...
    if (auto const hours_ago = (seconds % 86400) / 3600U; hours_ago > 0)
    {
        return fmt::format(
    ...
    if (auto const seconds_ago = seconds % 60; seconds_ago > 0)
    {
        return fmt::format(
...
std::string tr_format_time(time_t timestamp)
{
    ...
    if (auto const seconds = timestamp % 60; seconds > 0)
    {
        return fmt::format(
...
{
    if (auto const days_left = timestamp / 86400; days_left > 0)
    {
        return fmt::format(
    ...
    if (auto const hours_left = (timestamp % 86400) / 3600; hours_left > 0)
    {
        return fmt::format(
    ...
    if (auto const minutes_left = (timestamp % 3600) / 60; minutes_left > 0)
    {
        return fmt::format(
    ...
    if (auto const seconds_left = timestamp % 60; seconds_left > 0)
    {
        return fmt::format(
