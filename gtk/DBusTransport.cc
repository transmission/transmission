...
auto const MethodAddMetainfo = Glib::ustring{ TR_INTEROP_METHOD_ADD_METAINFO };
auto const MethodPresentWindow = Glib::ustring{ TR_INTEROP_METHOD_PRESENT_WINDOW };
auto const MethodPresentWindowWithToken = Glib::ustring{ TR_INTEROP_METHOD_PRESENT_WINDOW_WITH_TOKEN };
auto const MethodConfigDir = Glib::ustring{ TR_INTEROP_METHOD_CONFIG_DIR };
...
{
    return { std::data(sv), std::size(sv) };
}
...
public:
    ...
    [[nodiscard]] tr::interop::Reply present_window(std::string_view const activation_token) override
    {
        return tr::interop::present_window_via(
            ...
            { return call_reply(MethodPresentWindowWithToken, gtr_variant_tuple(to_ustring(token))); },
    ...
    {
        return call_reply(MethodAddMetainfo, gtr_variant_tuple(to_ustring(metainfo)));
    }
    ...
    [[nodiscard]] std::string config_dir() override
    {
        ...
        auto const dir = call<Glib::ustring>(MethodConfigDir, {});
...
private:
    ...
    template<typename T>
    [[nodiscard]] std::optional<T> call(Glib::ustring const& method, Glib::VariantContainerBase const& params)
    {
        return first_value<T>(connection_, path_, interface_, method, params, service_);
    }
    ...
    [[nodiscard]] tr::interop::Reply call_reply(Glib::ustring const& method, Glib::VariantContainerBase const& params)
    {
        try
        {
            auto const value = first_value_or_throw<bool>(connection_, path_, interface_, method, params, service_);
            if (!value)
            {
    ...
    Glib::RefPtr<Gio::DBus::MethodInvocation> const& invocation)
{
    ...
    if (method_name == MethodPresentWindow)
    {
        reply_bool(published_ != nullptr && published_->present_window({}) == tr::interop::Reply::Yes);
    }
    else if (method_name == MethodPresentWindowWithToken)
    {
    ...
    else if (method_name == MethodAddMetainfo)
    {
    ...
    else if (method_name == MethodConfigDir)
    {
