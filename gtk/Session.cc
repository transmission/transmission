...
{
    auto const mode = settings_->get_string(ClientPrefs::Key::SortMode);
    bool const is_reversed = settings_->get_boolean(ClientPrefs::Key::SortReversed);
    ...
    if (mode == ClientPrefs::SortMode::Activity)
    {
        sort_func = &compare_by_activity;
    }
    else if (mode == ClientPrefs::SortMode::Age)
    {
        sort_func = &compare_by_age;
    }
    else if (mode == ClientPrefs::SortMode::Progress)
    {
        sort_func = &compare_by_progress;
    }
    else if (mode == ClientPrefs::SortMode::Queue)
    {
        sort_func = &compare_by_queue;
    }
    else if (mode == ClientPrefs::SortMode::TimeLeft)
    {
        sort_func = &compare_by_eta;
    }
    else if (mode == ClientPrefs::SortMode::Ratio)
    {
        sort_func = &compare_by_ratio;
    }
    else if (mode == ClientPrefs::SortMode::State)
    {
        sort_func = &compare_by_state;
    }
    else if (mode == ClientPrefs::SortMode::Size)
    {
...
{
    auto const dirname = Glib::locale_from_utf8(settings_->get_string(ClientPrefs::Key::WatchDir));
...
{
    bool const is_enabled = settings_->get_boolean(ClientPrefs::Key::WatchDirEnabled);
    auto const dir = Gio::File::create_for_path(Glib::locale_from_utf8(settings_->get_string(ClientPrefs::Key::WatchDir)));
    ...
    , settings_(settings)
{
    ...
    settings_->signal_changed(ClientPrefs::Key::SortMode).connect(sigc::hide<0>(sigc::mem_fun(*this, &Impl::update_sort_mode)));
    settings_->signal_changed(ClientPrefs::Key::SortReversed)
        .connect(sigc::hide<0>(sigc::mem_fun(*this, &Impl::update_sort_mode)));
    settings_->signal_changed(ClientPrefs::Key::WatchDir).connect(sigc::hide<0>(sigc::mem_fun(*this, &Impl::watchdir_update)));
    settings_->signal_changed(ClientPrefs::Key::WatchDirEnabled)
        .connect(sigc::hide<0>(sigc::mem_fun(*this, &Impl::watchdir_update)));
    settings_->signal_changed(ClientPrefs::Key::InhibitDesktopHibernation)
...
tr_torrent* Session::Impl::create_new_torrent(tr_ctor* ctor)
{
    ...
    if (tor != nullptr && do_trash)
    {
        ...
        if (source != nullptr)
        {
            ...
            if (!is_internal)
            {
                gtr_file_trash_or_remove(source, settings_->get_boolean(ClientPrefs::Key::TrashCanEnabled), nullptr);
            }
        }
    }
...
bool Session::Impl::add_from_url(Glib::ustring const& url)
{
    ...
    auto const do_prompt = settings_->get_boolean(ClientPrefs::Key::ShowOptionsWindow);
...
void Session::Impl::remove_torrent(tr_torrent_id_t id, bool delete_files)
{
    ...
    if (tor != nullptr)
    {
        ...
        /* remove the torrent */
        auto const use_trash_can = settings_->get_boolean(ClientPrefs::Key::TrashCanEnabled);
        tr_torrentRemove(
            tor,
            delete_files,
...
void Session::Impl::maybe_inhibit_hibernation()
{
    ...
    bool const hibernation_allowed = !settings_->get_boolean(ClientPrefs::Key::InhibitDesktopHibernation) ||
        get_active_torrent_count() == 0;
